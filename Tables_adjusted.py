...
#            namefield=data.keys()[0]
    ...
    def redrawTable(self, event=None, callback=None):
        ...
        if self.paging == 0 and self.rows >= self.rowsperpage:
            self.paging = 1
    ...
    def paging_Off(self):
        self.rows=self.model.getRowCount()
        if self.rows >= self.rowsperpage:
            tkMessageBox.showwarning("Warning",
                                     'This table has over {0} rows.'
                                     'You should stay in page view.'
                                     'You can increase the rows per page in settings.'.format(self.rowsperpage),
    ...
    def popupMenu(self, event, rows=None, cols=None, outside=None):
        ...
        defaultactions = {"Set Fill Color" : lambda : self.setcellColor(rows,cols,key='bg'),
                        ...
                        "Export spreadsheet": self.exportTable,
        ...
        filecommands = ['New','Load','Save','Import text','Export spreadsheet']
